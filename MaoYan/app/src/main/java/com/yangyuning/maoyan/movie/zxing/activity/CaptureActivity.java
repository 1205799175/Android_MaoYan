package com.yangyuning.maoyan.movie.zxing.activity;
...
public class CaptureActivity extends Activity implements Callback {
	...
	private ImageView popop_menu;//PopopMenu
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		popop_menu = (ImageView) findViewById(R.id.qr_code_menu);
	...
	private void initListener() {
		//弹出PopopMenu
		popop_menu.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				PopupMenu popup = new PopupMenu(CaptureActivity.this, popop_menu);
				popup.getMenuInflater().inflate(R.menu.activity_qr_code, popup.getMenu());
	...
	private com.google.zxing.Result  parseQRcodeBitmap(String bitmapPath){
		...
		//如果我们把inJusDecodeBounds设为true, 那么BitmapFactory.decodeFile(String path, Option opt)
		//并不会真的返回一个Bitmap给你, 它仅仅会把宽高取回来给你
		options.inJustDecodeBounds = true;
		//此时的bitmap是null, 这段代码之后, option.outWidth和option.outHeight,就是我们想要的宽高了
		Bitmap bitmap = BitmapFactory.decodeFile(bitmapPath,options);
		//我们现在想取出来的图片的边长(二维码是正方形的)设置为400像素
		/**
		 ...
		 */
		//以上这种做法, 虽然把bitmap限定到了我们想要的大小, 但是并没有节约内存, 如果要节约内存我们还需要使用inSimpleSize这个属性
		options.inSampleSize = options.outHeight / 400;
		if(options.inSampleSize <= 0){
			options.inSampleSize = 1; //∑¿÷π∆‰÷µ–°”⁄ªÚµ»”⁄0
		}
		/**
		 * 辅助节约内存设置
		 *
		 * options.inPreferredConfig = Bitmap.Config.ARGB_4444;    // ƒ¨»œ «Bitmap.Config.ARGB_8888
